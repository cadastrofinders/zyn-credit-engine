...
INCREMENTAL_PROMPT = """\
ATUALIZAÇÃO RÁPIDA para: {tomador} | {tipo_operacao}

Rating atual: {rating_nota} | DSCR: {dscr} | LTV: {ltv} | Parecer: {parecer}
KPIs atuais: Receita {receita} | EBITDA {ebitda} | Margem {margem} | Dív/EBITDA {div_ebitda}

DOCUMENTO COMPLEMENTAR:
{novos_dados}

Responda JSON com APENAS os campos que mudaram. Exemplo:
{{"rating_final": {{"nota": "A", "parecer": "Favorável", "justificativa": "..."}}, "kpis": {{"receita_liquida": 600000000}}}}

Se o documento não altera nenhum indicador, responda: {{"sem_alteracao": true, "observacao": "..."}}
...
def analyze_incremental(
    ...
    """Atualiza análise existente com dados de documentos complementares (incremental rápido)."""
    client = _get_client()
    ...
    _status("Análise incremental rápida...")

    # Extract summary from previous analysis (minimal tokens)
    rating = analise_anterior.get("rating_final", {})
    kpis = analise_anterior.get("kpis", {})
    ...
    user_prompt = INCREMENTAL_PROMPT.format(
        tomador=parametros_operacao.get("tomador", "N/I"),
        tipo_operacao=parametros_operacao.get("tipo_operacao", "N/I"),
        rating_nota=rating.get("nota", "N/I"),
        dscr=rating.get("dscr_projetado", kpis.get("dscr", "N/I")),
        ltv=rating.get("ltv_efetivo", kpis.get("ltv", "N/I")),
        parecer=rating.get("parecer", "N/I"),
        receita=kpis.get("receita_liquida", "N/I"),
        ebitda=kpis.get("ebitda", "N/I"),
        margem=kpis.get("margem_ebitda", "N/I"),
        div_ebitda=kpis.get("divida_liquida_ebitda", "N/I"),
        novos_dados=novos_formatados,
    )

    _status(f"Enviando ~{len(user_prompt) // 4:,} tokens para {MODEL}...")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            with client.messages.stream(
                model=MODEL,
                max_tokens=4096,
                system="Analista de crédito ZYN Capital. Responda SOMENTE JSON com campos alterados pelo documento complementar. Seja conciso.",
                messages=[{"role": "user", "content": user_prompt}],
            ) as stream:
                for text in stream.text_stream:
                    response_text += text
            _status(f"Resposta: {len(response_text):,} chars")
            break
    ...
    delta = _parse_json(response_text)

    # Merge delta into previous analysis (deep merge)
    resultado = json.loads(json.dumps(analise_anterior, ensure_ascii=False))

    if not delta.get("sem_alteracao"):
        for key, value in delta.items():
            if isinstance(value, dict) and isinstance(resultado.get(key), dict):
                resultado[key].update(value)
            else:
                resultado[key] = value
    ...
    investors = match_investors(
        ...
        setor=setor,
        garantias=garantias_list,
        rating=resultado.get("rating_final", {}).get("nota"),
        prazo_anos=parametros_operacao.get("prazo_meses", 0) / 12 if parametros_operacao.get("prazo_meses") else None,
        top_n=30,
    )
    resultado["investor_matching"] = {
        "setor_detectado": setor,
        "investidores_sugeridos": investors,
        "total_matches": len(investors),
    }
    resultado["_setor"] = setor

    _status("Análise incremental concluída.")
    return resultado
